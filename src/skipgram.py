...
class SkipGramNeg(nn.Module):
    ...
    def __init__(self, n_vocab: int, n_embed: int, noise_dist: torch.Tensor = None):
        ...
        #TODO
        self.in_embed: nn.Embedding = torch.nn.Embedding(n_vocab, n_embed)
        self.out_embed: nn.Embedding = torch.nn.Embedding(n_vocab, n_embed)
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        # TODO
        noise_words: torch.Tensor = torch.multinomial(noise_dist, num_samples=n_samples*batch_size, replacement=True).view(batch_size, n_samples)
        ...
        # TODO
        noise_vectors: torch.Tensor = self.out_embed(noise_words).to(device)
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        out_loss = torch.log(torch.sigmoid(input_vectors @ output_vectors.T)).sum()
        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        # print(f"{torch.bmm(noise_vectors, input_vectors.T) = }")
